...
- 主节点支持添加、启动、停止、配置自启动和移除转发规则；运行态与自启动策略独立。
- 转发状态只反映 SSH 主机连接与本机监听，不探测目标服务端口是否已启动。
- 转发记录引用从节点仅为了确定 SSH 目标和远端端口。
- 从节点 APR 不保存主节点转发状态。
- `forward-only` 主机不是从节点服务目录，不提供远程服务启停。
...
- APR 重启时开启自启动的转发规则会在原本机端口恢复；APR 运行期间，当前应运行的
  规则在 AutoSSH 父进程退出后也会恢复。
- 移除转发规则会停止其 APR 托管进程并删除记录。
