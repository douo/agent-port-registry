...
- [x] 节点详情可添加、启动、停止、配置自启动和移除任意转发规则。
- [x] 启停不修改自启动策略；APR 启动只拉起开启自启动的规则。
- [x] 后台监督恢复本次 APR 运行期间当前应运行的规则。
- [x] 转发规则不探测目标服务端口；服务是否启动不影响 SSH 转发状态。
- [x] `forward-only` 主机不暴露远程服务管理能力。
- [x] macOS LaunchAgent 通过用户登录且交互式 shell 启动，Homebrew 工具和用户终端环境可见。
